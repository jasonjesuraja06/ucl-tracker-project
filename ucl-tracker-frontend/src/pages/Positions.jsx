...
export default function Positions() {
  ...
  useEffect(() => {
    api.get('/players/positions')
      .then(res => {
        console.log('API response for positions:', res.data);
  ...
  const filtered = positions
    .map(p => {
      const name = parsePosition(p);
      const slug = p;
      return { original: p, name, slug };
    })
  ...
  return (
    <div className="min-h-screen bg-black p-6">
      <div className="container mx-auto">
        <h1 className="text-4xl font-bold text-green-400 mb-8 text-center drop-shadow-[0_2px_6px_rgba(0,255,0,0.6)]">Positions</h1>
        <input 
          type="text" 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filtered.slice(0, visible).map(({ original, name, slug }) => (
            <Link to={`/positions/${slug}`} key={original} className="block">
              <div className="bg-gray-900/80 p-6 rounded-xl shadow-lg hover:shadow-green-500/20 transition-all duration-300 border border-green-500/20 text-center">
                <img src={`/position-images/${slug}.png`} alt={name} className="w-48 h-48 mx-auto mb-4 rounded-full border-2 border-green-500/30" onError={(e) => e.target.src = '/position-images/default.png'} />
